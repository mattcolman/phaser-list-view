...
var Scroller = function () {
  ...
  _createClass(Scroller, [{
    ...
    value: function handleDown(target, pointer) {
      ...
      this.events.onInputDown.dispatch({ target: target, pointer: pointer });
    }
  }, {
    key: 'handleMove',
    value: function handleMove(pointer, x, y) {
      ...
      var _distance = this.down - o[this.o.direction];
    ...
    key: 'requestDiff',
    value: function requestDiff(diff, target, min, max, overflow) {
      var scale = 0;
      if (target > max) {
    ...
    value: function handleUp(target, pointer) {
      ...
      // *** END LIMITS
      var _distance = this.down - pointer[this.o.direction];
      var duration = 1;
      ...
      if (this.scrollObject[this.o.direction] > this.max) {
        ...
        // *** DURATION
        var distance = Math.abs(this.target - this.scrollObject[this.o.direction]);
        duration = this.getDuration(distance);

        this.doTween(duration, this.target);
      }

      this.events.onInputUp.dispatch({ target: target, pointer: pointer });
    }
  }, {
    key: 'getDuration',
    ...
    key: 'snap',
    value: function snap(target) {
      if (!this.o.snapping) {
        return target;
      }
      return _math_utils2.default.nearestMultiple(target, this.o.snapStep);
    }
  }, {
    key: 'addMomentum',
    value: function addMomentum(target, touchTime, offset) {
    ...
    key: 'doTween',
    value: function doTween(duration, target) {
    ...
    value: function handleUpdate() {
      ...
      this.dispatchValues.percent = _math_utils2.default.percentageBetween2(this.scrollObject[this.o.direction], this.o.from, this.o.to);
